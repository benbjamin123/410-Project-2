...
void rbInsert(struct node **root, char *data, int key)
{
...
//Written by Ben
//Search the tree for a specific url and return the node containing it
struct node *rbSearch(struct node *root, char *url){
    if (root == NULL)
        return NULL;
    if(strcmp(root->data, url) == 0){
	return root;
    }
    else if(strcmp(root->data, url) > 0){
	return rbSearch(root->left, url);
    }
    else if(strcmp(root->data, url) < 0){
	return rbSearch(root->right, url);
    }
}
